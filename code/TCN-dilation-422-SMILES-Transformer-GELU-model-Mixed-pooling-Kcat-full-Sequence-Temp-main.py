...
class ConditionFloatRBF(nn.Module):
    def __init__(self, embed_dim, device=None):
        ...
        self.device = device or torch.device('cpu')
        # 仅保留温度（Temp）的RBF参数
        self.rbf_params = {'Temp': (np.arange(0, 100, 1), 10.0)}
        self.condition_names = ["Temp"]
...
def extract_temp_features(temp_values, device):
    rbf_model = ConditionFloatRBF(embed_dim=500, device=device)
    temp_embed = rbf_model({"Temp": temp_values})
    return temp_embed
...
class SubstrateDataset(Dataset):
    def __init__(self, df, device):
        ...
        self.y = df['log10_Kcat'].astype(float).values
    ...
    def __getitem__(self, idx):
        ...
        # 仅提取温度特征
        temp_embed = self.rbf_model({"Temp": temp_values})
        return smiles_embedding, seq_vector, temp_embed, y
...
class MLPModel(nn.Module):
    def __init__(self, smiles_embedding_dim, seq_fc_dim, temp_dim, hidden_dims, output_dim, dropout_rate=0.3):
        ...
        layers = []
        # 输入维度仅包含温度特征，移除pH维度
        input_dim = smiles_embedding_dim + seq_fc_dim + temp_dim
    ...
    def forward(self, smiles_embedding, seq_vector, temp_embed):
        ...
        # 仅拼接温度特征
        combined_input = torch.cat([smiles_embedding, seq_vector, temp_embed], dim=1)
...
def train(model, criterion, optimizer, train_loader, device):
    ...
    for smiles_embedding, seq_vector, temp_embed, batch_y in train_loader:
        smiles_embedding, seq_vector, temp_embed, batch_y = smiles_embedding.to(device), seq_vector.to(
            device), temp_embed.to(device), batch_y.to(device)
        optimizer.zero_grad()
        outputs = model(smiles_embedding, seq_vector, temp_embed)
...
def validate(model, criterion, val_loader, device):
    ...
    with torch.no_grad():
        for smiles_embedding, seq_vector, temp_embed, batch_y in val_loader:
            smiles_embedding, seq_vector, temp_embed, batch_y = smiles_embedding.to(device), seq_vector.to(
                device), temp_embed.to(device), batch_y.to(device)
            outputs = model(smiles_embedding, seq_vector, temp_embed)
...
def test(model, criterion, test_loader, device):
    ...
    with torch.no_grad():
        for smiles_embedding, seq_vector, temp_embed, batch_y in test_loader:
            smiles_embedding, seq_vector, temp_embed, batch_y = smiles_embedding.to(device), seq_vector.to(
                device), temp_embed.to(device), batch_y.to(device)
            outputs = model(smiles_embedding, seq_vector, temp_embed)
            # 检查 outputs 是否为单个值
            if outputs.numel() == 1:
                test_predictions.append(outputs.squeeze().cpu().item())
            else:
...
def main():
    ...
    train_df = pd.read_csv(
        '/home/lyy/lyy/All_Sequence/data/Kcat/final_train_data.csv')
    val_df = pd.read_csv(
        '/home/lyy/lyy/All_Sequence/data/Kcat/final_val_data.csv')
    test_df = pd.read_csv(
        '/home/lyy/lyy/All_Sequence/data/Kcat/final_test_data.csv')
    ...
    # 移除了pH维度参数
    model = MLPModel(smiles_embedding_dim=smiles_embedding_dim, seq_fc_dim=seq_fc_dim, temp_dim=temp_dim,
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            best_epoch = epoch + 1
            # 更新保存路径，反映当前模型只使用温度特征
            torch.save(model.state_dict(),
                       '/home/lyy/lyy/All_Sequence/model/24.11.28/TCN-dilation4-2-2-kernel_size5-SMILES-Transformer-MLP-GELU-model-Mixed-pooling-Kcat-full-Sequence_Temp_only_dim500_lr0.001_aa1000_best_model_file_model.pth')
            patience_counter = 0
        else:
            patience_counter += 1
    ...
    with open(
            # 更新日志文件路径，反映当前模型只使用温度特征
            '/home/lyy/lyy/All_Sequence/result/24.11.28/TCN-dilation4-2-2-kernel_size5-SMILES-Transformer-MLP-GELU-model-Mixed-pooling-full-Sequence_Temp_only_dim500_lr0.001_aa1000_training_log.txt',
            'w') as f:
        f.write(
